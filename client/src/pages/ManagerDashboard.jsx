...
import { ClipboardCheck, AlertTriangle, Eye, Send, MapPin, Clock, User, FileEdit, Package, X, ChevronDown, Plus, Trash2 } from 'lucide-react';
...
const ManagerDashboard = () => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (fillIncidentId) {
        const found = loadedIncidents.find(i => i._id === fillIncidentId);
        if (found) {
          setFillTarget(found);
  ...
  const normalPending  = incidents.filter(i => !i.pendingManagerAssist);
